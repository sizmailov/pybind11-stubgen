...
if __name__ == "__main__":
    ...
    rel_expected_stubs = sorted(
        [p.relative_to(expected_path) for p in expected_path.rglob("*.pyi")]
    )
    rel_generated_stubs = sorted(
        [p.relative_to(generated_path) for p in generated_path.rglob("*.pyi")]
    )
